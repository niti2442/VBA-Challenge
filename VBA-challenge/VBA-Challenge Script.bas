...
'Loop for summary
For i = 2 To LastRowI
'For greatest total volume, check if next value is larger, if yes then take over a new value and populate ws.cells
...
GreatVol = GreatVol
End If
'For greatest increase, check if next value is larger, if yes then take over a new value and populate ws.cells
...
GreatIncr = GreatIncr
End If
'For greatest decrease, check if next value is smaller, if yest then take over a new value and populate ws.cells
If ws.Cells(i, 11).Value < GreatDecr Then
GreatDecr = ws.Cells(i, 11).Value
ws.Cells(3, 16).Value = ws.Cells(i, 9).ValueN
Else
GreatDecr = GreatDecr
End If
